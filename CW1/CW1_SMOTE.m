...
corr(target_process, data_process)

rng(42);

% Splitting the data into training(for SMOTE processing and NN) and
% testing(for inference evaluation of trained NN model)

cv = cvpartition(target_process, 'HoldOut', 0.2);

Xtrain = data_process(training(cv), :);
Ytrain = target_process(training(cv));

Xtest = data_process(test(cv), :);
Ytest = target_process(test(cv));

% Applying SMOTE

rng(42)
[meas1,species1,new_meas,new_species] = smote(Xtrain, [], 'Class', Ytrain);

us = unique(Ytrain);
K = groupcounts(Ytrain);
...
for ii = 1:numel(us)
    ...
    I = Ytrain == us(ii);
    plot(Xtrain(I,2), Xtrain(I,4), '.');
    legend_str{end+1} = sprintf('Class %d (N=%d)', us(ii), K(ii));
    
    I = new_species == us(ii);
    if sum(I) > 0
        plot(new_meas(I,2), new_meas(I,4), 'o');
        legend_str{end+1} = sprintf('Synthesized Class %d', us(ii));
    end
end
...
IN = meas1';
TARGET = full(ind2vec(species1' + 1));
...
[~, pred_class] = max(Ypred);
pred_class = pred_class - 1;

score = sum(Ytest == pred_class')/length(Ytest);
score = score*100;

figure; hold off
confusionchart(Ytest, pred_class');

% Depending on which model was trained comment one and uncomment the other

% title(sprintf('SCG Accuracy Score %.2f%%', score)); % if SCG (Scaled Conjugate Gradient) algorithm was used
title(sprintf('LMA Accuracy Score %.2f%%', score)); % if Levenberg-Marquardt algorithm was used
